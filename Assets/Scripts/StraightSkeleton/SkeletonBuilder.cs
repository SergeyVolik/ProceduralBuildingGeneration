...
using StraightSkeleton.Roof.Events;
using StraightSkeleton.Roof.Events.Chains;
using StraightSkeleton.Roof.Path;
...
namespace StraightSkeleton
{
    ...
    public class SkeletonBuilder
    {
        ...
        public static SkeletonRoofResult BuildRoof(List<Vector2d> polygon)
        {
            return Build(polygon, null);
        }
        public static SkeletonCorridorResult BuildCorridor(List<Vector2d> polygon, CorridorWidth corridorWidth, OutputType type)
        {
            var sr = Build(polygon, null);
            var sc = new SkeletonCorridor(sr, corridorWidth, type);
            return sc.GetCorridorResult();
        }
        public static SkeletonCorridorResult BuildCorridor(SkeletonRoofResult roof, List<Vector2d> polygon, CorridorWidth corridorWidth, OutputType type)
        {        
            var sc = new SkeletonCorridor(roof, corridorWidth, type);
            return sc.GetCorridorResult();
        }

        /// <summary> Creates straight skeleton for given polygon with holes. </summary>
        public static SkeletonRoofResult Build(List<Vector2d> polygon, List<List<Vector2d>> holes)
        {
            //если полигон задан по часовой то делаем реверс
            polygon = InitPolygon(polygon);
            //если дыры полигона противчасовой делаем реверс
        ...
        private static SkeletonRoofResult AddFacesToOutput(List<FaceQueue> faces)
        {
            ...
            return new SkeletonRoofResult(edgeOutputs, distances);
        }

        //c)  для невыпуклых вершин найдём также точки пересечения с противолежащими 
        //    рёбрами (как это описывалось раньше) и положим в приоритетную очередь ближайшую точку пересечения I.
        //    Будем также с этой точкой хранить её тип — edge event или split event.
        ...
        //Для каждой биссектрисы выпуклой вершины найдём ближайшую точку пересечения с биссектрисой
        //соседней вершины
