...
namespace StraightSkeleton.Primitives
{
    internal static class PrimitiveUtils
    {
        ...
        public static bool IsPointOnBoarder(Vector2d point, List<Vector2d> points)
        {          
            var segment = new LineSegment2d(points[0], points[points.Count-1]);
            if (segment.Contains(point))
                return true;

            for (var i = 0; i < points.Count-1; i++)
            {
                segment = new LineSegment2d(points[i], points[i + 1]);
                if (segment.Contains(point))
                    return true;
            }
            return false;
